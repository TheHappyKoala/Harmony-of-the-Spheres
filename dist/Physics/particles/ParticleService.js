...
export default class {
    static getShapeOfParticles(number, minD, maxD, callback) {
        const particles = [];
        for (let i = 0; i < number; i++)
            callback(particles, minD, maxD);
        return particles;
    }
    static getDiscParticle(particles, minD, maxD) {
    ...
    static getSphereParticle(particles, minD, maxD) {
    ...
    static getCubeParticle(particles, minD, maxD, cube) {
        particles.push({
    ...
    static getParticleSystem(vectors, tilt, primary, mRange, g, withOrbit) {
        const p = new H3();
        const v = new H3();
...
//# sourceMappingURL=ParticleService.js.map