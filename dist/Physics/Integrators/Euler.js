...
export default class {
    ...
    insertMassInTree(mass, tree) {
        ...
        if (nChildren == 0) {
            ...
            for (let i = 0; i < 8; i++) {
                if (this.isInTree({ x: mass.x, y: mass.y, z: mass.z }, tree.children[i])) {
                    const v = new H3();
                    tree.CoM = v
                        .set(tree.CoM)
    ...
    BHAccelerate(p, tree) {
        ...
        if (nChildren == 0) {
            ...
            const acc = rVector
                .multiplyByScalar(other.m * this.g / Math.pow(r * r + this.softeningSquared, 1.5))
                .toObject();
            return acc;
        }
        else if (nChildren == 8) {
            ...
            if (tree.size / r < this.theta) {
                const acc = rVector
                    .multiplyByScalar(this.g * tree.mass / Math.pow(r * r + this.softeningSquared, 1.5))
                    .toObject();
                return acc;
            }
            else {
    ...
    BHGenerateAccelerationVectors(p, tree) {
        const nP = p.length;
        const acc = [];
        for (let i = 0; i < nP; i++) {
