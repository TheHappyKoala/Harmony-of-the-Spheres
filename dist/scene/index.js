...
import { modifyScenarioProperty, deleteMass } from '../action-creators/scenario';
import H3 from '../Physics/vectors';
import getIntegrator from '../Physics/Integrators';
import { getObjFromArrByKeyValuePair } from '../utils';
import { getDistanceParams, getOrbit, getRandomNumberInRange, setBarycenter, clampAbs, getEllipse } from '../Physics/utils';
...
export default {
    init(webGlCanvas, graphics2DCanvas) {
        ...
        this.graphics2D = new Graphics2D(graphics2DCanvas).setDimensions(this.w, this.h);
        ...
        this.camera = new Camera(45, this.w / this.h, this.scenario.logarithmicDepthBuffer ? 1e-9 : 1, this.scenario.logarithmicDepthBuffer ? 1e27 : 1500000000000, this.graphics2D.canvas);
        ...
        this.ellipseCurve = new CustomEllipseCurve(0, 0, 200, 200, 0, 2 * Math.PI, false, 0, 500, 'limegreen');
        ...
        this.manager.onProgress = url => store.dispatch(modifyScenarioProperty({
    ...
    diffMasses(previousMasses, newMasses) {
        if (newMasses.length < previousMasses.length) {
            ...
            while (i < previousMasses.length) {
                ...
                if (newMasses.some(entry2 => entry1.name === entry2.name))
                    ++i;
                else {
    ...
    updateEllipseCurve() {
        ...
        if (scenario.isMassBeingAdded) {
            ...
            const primary = getObjFromArrByKeyValuePair(scenario.masses, 'name', scenario.primary);
            const a = scenario.a;
            const e = scenario.e;
            const w = scenario.w;
            const i = scenario.i;
            ...
            this.ellipseCurve.update((this.rotatingReferenceFrame.x - primary.x - ellipse.focus) * scale, (this.rotatingReferenceFrame.y - primary.y) * scale, ellipse.xRadius * scale, ellipse.yRadius * scale, 0, 2 * Math.PI, false, 0, { x: 0, y: i, z: w });
        }
        else
            this.ellipseCurve.visible = false;
    },
    collisionCallback(looser, survivor) {
        ...
        if (this.scenario.cameraFocus === looser.name ||
            ...
            store.dispatch(modifyScenarioProperty({
                key: 'freeOrigo',
                value: {
            ...
            }, { key: 'primary', value: survivor.name }, { key: 'rotatingReferenceFrame', value: survivor.name }, { key: 'cameraPosition', value: 'Free' }, { key: 'cameraFocus', value: 'Origo' }));
        }
        ...
        if (survivingManifestation.materialShader) {
            survivingManifestationRotation = survivingManifestation.getObjectByName('Main').rotation;
            hitPoint = CollisionsService.getClosestPointOnSphere(new H3().set({
            ...
            uniforms.impacts.value[impactIndex].impactPoint.set(-hitPoint.x, -hitPoint.y, -hitPoint.z);
        ...
        const totalWithAddedFragments = this.particlePhysics.particles + numberOfFragments;
        const excessFragments = this.scenario.particles.max - totalWithAddedFragments;
        ...
        const kineticVelocity = {
            x: CollisionsService.convertKineticEnergyToVelocityComponent(looser, fragmentMass, 0.00000000001, 'vx'),
            y: CollisionsService.convertKineticEnergyToVelocityComponent(looser, fragmentMass, 0.00000000001, 'vy'),
            z: CollisionsService.convertKineticEnergyToVelocityComponent(looser, fragmentMass, 0.00000000001, 'vz')
        };
        ...
        for (let i = 0; i < numberOfFragments; i++) {
            ...
            const particleVelocity = new H3()
                .set(CollisionsService.getDeflectedVelocity(survivor, Object.assign({}, looser, { vx: kineticVelocity.x, vy: kineticVelocity.y, vz: kineticVelocity.z })))
                .rotate({ x: 1, y: 0, z: 0 }, angleX)
                .rotate({ x: 0, y: 1, z: 0 }, angleY)
                .rotate({ x: 0, y: 0, z: 1 }, angleZ)
                .add({ x: survivor.vx, y: survivor.vy, z: survivor.vz });
            this.particlePhysics.particles.push(Object.assign({}, new H3()
                .set({
                ...
                .toObject(), { vx: particleVelocity.x / clampAbs(1, maxAngle, angleX / 10), vy: particleVelocity.y / clampAbs(1, maxAngle, angleY / 10), vz: particleVelocity.z / clampAbs(1, maxAngle, angleZ / 10) }));
        }
    },
    loop() {
        this.scenario = store.getState().scenario;
        ...
        const { rotatingReferenceFrame, cameraPosition, cameraFocus, freeOrigo, barycenterMassOne, barycenterMassTwo } = this.scenario;
        ...
        if (this.scenario.background && !arena.visible)
            arena.visible = true;
        if (!this.scenario.background && arena.visible)
            arena.visible = false;
        ...
        if (this.scenario.playing)
            this.system.iterate();
        dt = this.system.dt;
        setBarycenter(this.scenario.systemBarycenter
            ? this.system.masses
            : this.system.masses
                .map(mass => {
                if (mass.name === barycenterMassOne ||
                    mass.name === barycenterMassTwo)
                    return mass;
            })
                .filter(mass => mass !== undefined), this.barycenterPosition);
        ...
        const barycenterPositionScaleFactor = rotatingReferenceFrame !== 'Barycenter' ? this.scenario.scale : 1;
        ...
        if (this.scenario.barycenter)
            this.graphics2D.drawLabel('Barycenter', this.utilityVector.set(this.barycenterPosition.x, this.barycenterPosition.y, this.barycenterPosition.z), this.camera, cameraPosition === 'Free' ? true : false, cameraFocus === 'Barycenter' ? true : false, 'left', 'limegreen', drawBaryCenterLabel);
        ...
        if (this.previousCameraFocus !== cameraFocus &&
            ...
            if (cameraPosition === 'Free') {
                ...
                if (cameraFocus === 'Barycenter') {
                    this.camera.position.set(this.barycenterPosition.x, this.barycenterPosition.y, this.barycenterPosition.z + 100000000);
                    this.camera.lookAt(...barycenterPositionArray);
                }
            }
        }
        for (let i = 0; i < this.massManifestations.length; i++) {
            ...
            const cameraDistanceToFocus = Math.sqrt(getDistanceParams(this.camera.position, this.manifestationPosition)
                .dSquared);
            massManifestation.draw(this.manifestationPosition.x, this.manifestationPosition.y, this.manifestationPosition.z, this.camera.position, cameraDistanceToFocus);
            ...
            if (dt < 0.0002)
                zOffset = radius * 80;
            if (dt > 0.0002)
                zOffset = radius < 18 ? 60000 : radius * 1500;
            if (dt > 0.5)
                zOffset = radius !== 1.2 ? radius * 500000 : 4000000;
            if (trail) {
                if (trail.visible) {
                ...
                if (!trail.visible || !massManifestation.visible) {
                    if ((cameraPosition === 'Free' && cameraFocus !== name) ||
                        (cameraPosition === 'Free' &&
                            cameraFocus === name &&
                            cameraDistanceToFocus > zOffset * 0.5)) {
                        trail.visible = true;
            ...
            if (this.previousCameraFocus !== cameraFocus && cameraFocus === name) {
                this.previousCameraFocus = cameraFocus;
                if (cameraPosition === 'Free') {
                    this.camera.position.set(this.manifestationPosition.x, this.manifestationPosition.y, this.manifestationPosition.z + zOffset);
                    this.camera.lookAt(...manifestationPositionArray);
                }
            }
            if (this.scenario.labels && cameraPosition !== name)
                this.graphics2D.drawLabel(name, this.utilityVector.set(...manifestationPositionArray), this.camera, cameraPosition === 'Free' ? true : false, cameraFocus === name ? true : false, 'right', 'white', drawMassLabel);
            ...
            if (cameraPosition === 'Chase' && cameraFocus === name) {
                ...
                this.camera.position.set(...new H3()
                    .set({
            ...
            if (mass.spacecraft && this.scenario.playing) {
                const directionOfVelocity = new THREE.Vector3((mass.x + mass.vx * dt) * this.scenario.scale, (mass.y + mass.vy * dt) * this.scenario.scale, (mass.z + mass.vz * dt) * this.scenario.scale);
        ...
        if (this.scenario.particles) {
            this.particles.draw(this.particlePhysics.particles, this.rotatingReferenceFrame);
            this.particles.twinklingParticles = this.scenario.twinklingParticles;
        }
        if (this.scenario.playing && this.scenario.collisions)
            CollisionsService.doCollisions(this.system.masses, this.scenario.scale, this.collisionCallback);
        ...
        store.dispatch(modifyScenarioProperty({
            ...
            key: 'maximumDistance',
            value: this.camera.getVisibleSceneHeight(this.camera.position.z) /
                2 /
                this.scenario.scale
        }));
        TWEEN.update();
    ...
    addRing() {
        for (let i = 0; i < this.scenario.particles.rings.length; i++) {
            ...
            const primary = ring.primary !== 'custom'
                ? getObjFromArrByKeyValuePair(this.scenario.masses, 'name', ring.primary)
                : ring.customPrimaryData;
            const generatedParticles = ParticleService.getParticleSystem(ParticleService.getShapeOfParticles(ring.number, ring.minD, ring.maxD, ParticleService.getDiscParticle), ring.tilt, primary, [0, 0], this.scenario.g, true);
