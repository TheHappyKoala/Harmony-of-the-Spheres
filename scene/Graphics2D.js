export default class {
    ...
    threeToTwo(p, camera, isOrbital, isTarget) {
        ...
        if ((isOrbital && isTarget) || isTarget)
            return { x: w / 2, y: h / 2 };
        p = p.project(camera);
        p.x = (p.x + 1) / 2 * w;
        p.y = -(p.y - 1) / 2 * h;
    ...
    drawLabel(name, p, camera, isOrbital, isTarget, placement, color, drawSymbolCallback) {
...
export function drawBaryCenterLabel(ctx, x, y, color) {
...
export function drawMassLabel(ctx, x, y, color) {
    ctx.strokeStyle = color;
    ctx.beginPath();
